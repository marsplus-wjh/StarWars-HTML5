...
function ok()
{
    ...
    zidan();
    // TimeA = clearInterval(TimeA);
    // TimeB = clearInterval(TimeB);
    // TimeE = clearInterval(TimeE);
    // TimeZ = clearInterval(TimeZ);
    TimeA = record();
    TimeB = finaldiv();
    TimeE = makeEnemy();
    TimeZ = zidan();
    ...
    {
        dan[i].parentNode.removeChild(dan[i]);
        clearInterval(dan[i].T);
    }
